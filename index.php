            ...
            <section class="content__side">
                ...
                <nav class="main-navigation">
                    <ul class="main-navigation__list">
                        <?php
                            $sqlProj = "SELECT tab, name_project FROM project";
                            $resultProj = mysqli_query($con, $sqlProj);
                        
                            $rowsProj = mysqli_fetch_all($resultProj, MYSQLI_ASSOC);
                        ?>  
                        
                        <?php foreach ($rowsProj as $row):?>
                        
                        <li class="main-navigation__list-item">
                            <a class="main-navigation__list-item-link" href="/?<?=$row['tab']?>"><?=$row['name_project']?></a>
            ...
            <main class="content__main">
                ...
                <table class="tasks">
                    <?php
                        ...
                        $contask = mysqli_connect("localhost", "root", "root", "lesson5");
                            if($contask==false) {
                        ...
                        $search = $_GET['name_task'] ?? '';
                        if($search) {
                            $sql = "SELECT name_task, date_task FROM task WHERE MATCH(name_task) AGAINST('')";
                            $stmt = db_get_prepare_stmt($contask, $sql, [$search]);
                    ...
                    <?php
                        ...
                        if (isset($_GET['tab']) && $_GET['tab'] == 'работа') {
                        
                            $qwe = 'SELECT name_task, date_task FROM task WHERE id=1';
                            $res = mysqli_query($con, $qwe);
                            $rowsqwe = mysqli_fetch_all($res, MYSQLI_ASSOC);
                            foreach ($rowsqwe as $roww) {
                               ...
                               print( '<tr class="tasks__item task">
                                    <td class="task__select">
                                        ...
                                        <input class="checkbox__input visually-hidden task__checkbox" type="checkbox" 
                                        >
                                        <span class="checkbox__text">'.$roww['name_task'].'</span>
                                        </label>
                                    </td>
                                    <td class="task__file">
                                        <a class="download-link" href="">Home.psd</a>
                        ...
                        if (isset($_GET['tab']) && $_GET['tab'] == 'раб-ота') {
                        
                            $qwe = 'SELECT name_task, date_task FROM task WHERE id=2';
                            $res = mysqli_query($con, $qwe);
                            $rowsqwe = mysqli_fetch_all($res, MYSQLI_ASSOC);
                            foreach ($rowsqwe as $roww) {
                        ...
                        if (isset($_GET['tab']) && $_GET['tab'] == 'учёба') {
                        
                            $qwe = 'SELECT name_task, date_task FROM task WHERE id=3';
                            $res = mysqli_query($con, $qwe);
                            $rowsqwe = mysqli_fetch_all($res, MYSQLI_ASSOC);
                            foreach ($rowsqwe as $roww) {
                        ...
                        if (isset($_GET['tab']) && $_GET['tab'] == 'входящие') {
                        
                            $qwe = 'SELECT name_task, date_task FROM task WHERE id=4';
                            $res = mysqli_query($con, $qwe);
                            $rowsqwe = mysqli_fetch_all($res, MYSQLI_ASSOC);
                            foreach ($rowsqwe as $roww) {
                        ...
                        if (isset($_GET['tab']) && $_GET['tab'] == 'домашние-дела') {
                            
                            
                            $qwe = 'SELECT name_task, date_task FROM task WHERE id=5';
                            $res = mysqli_query($con, $qwe);
                            $rowsqwe = mysqli_fetch_all($res, MYSQLI_ASSOC);
                            foreach ($rowsqwe as $roww) {
                        ...
                        if (isset($_GET['tab']) && $_GET['tab'] == 'домашние-дела') {
                            ...
                            $qwe = 'SELECT name_task, date_task FROM task WHERE id=6';
                            $res = mysqli_query($con, $qwe);
                            $rowsqwe = mysqli_fetch_all($res, MYSQLI_ASSOC);
                            foreach ($rowsqwe as $roww) {
